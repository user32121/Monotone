...
public class MonotoneCommand {
    public static void init() {
        makeCommand("visibility", node -> node.then(VisibilitySetting.SETTING_ARG.executes(VisibilitySetting::getHighlight).then(VisibilitySetting.VALUE_ARG.executes(VisibilitySetting::setHighlight)))
        ...
        makeCommand("limit", node -> node.then(RotationPlane.PLANE_ARG.executes(RotationPlane::getLimit).then(ClientCommandManager.literal("clear").executes(RotationPlane::clearLimit))
                .then(RotationPlane.MIN_ARG.then(RotationPlane.MAX_ARG.executes(RotationPlane::setLimit)
                        .then(RotationPlane.DEVIANCE_ARG.executes(RotationPlane::setLimitWithDeviance).then(RotationPlane.SOFTCAP_ARG.executes(RotationPlane::setLimitWithDevianceSoftcap)))))));
        makeCommand("tasks",
        ...
        makeCommand("mask", node -> node.then(InteractionMask.INTERACTION_ARG
                .then(ClientCommandManager.literal("add").then(ClientCommandManager.literal("cuboid").then(InteractionMask.FROM_ARG.then(InteractionMask.TO_ARG.executes(InteractionMask::addCuboid))))
    ...
    private static void makeCommand(String command, Function<LiteralArgumentBuilder<FabricClientCommandSource>, LiteralArgumentBuilder<FabricClientCommandSource>> buildCommand) {
        ClientCommandRegistrationCallback.EVENT.register((dispatcher, registry) -> dispatcher.register(buildCommand.apply(ClientCommandManager.literal(Monotone.MODID + ":" + command))));
    }
}
